  ...
  <style>
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .lab-logo {
      ...
      filter: brightness(2) contrast(0.8) drop-shadow(0 0 8px var(--orange-glow));
    }
    .brand {
      display: flex;
      align-items: center;
...
<body>
  ...
  <div class="quick-actions-wrapper">
    ...
    <div class="quick-actions-panel" id="quick-actions-panel" hidden>
      <div class="quick-actions-grid">
        <button class="qa-item clear-action" onclick="clearChat()">Clear Chat</button>
        <button class="qa-item" onclick="sendPromptFromPanel('show ram status and disk space')">RAM &amp; Disk</button>
        <button class="qa-item" onclick="sendPromptFromPanel('what is my system uptime?')">Uptime</button>
        <button class="qa-item" onclick="sendPromptFromPanel('show my tasks')">Tasks</button>
        <button class="qa-item" onclick="sendPromptFromPanel('check cpu info')">CPU Info</button>
        <button class="qa-item" onclick="sendPromptFromPanel('check cpu temperature')">Temperature</button>
        <button class="qa-item" onclick="sendPromptFromPanel('show disk space')">Disk Space</button>
        <button class="qa-item" onclick="sendPromptFromPanel('/memory')">Memory</button>
        <button class="qa-item" onclick="sendPromptFromPanel('/memory search')">Search History</button>
        <button class="qa-item" onclick="sendPromptFromPanel('summarize &quot;experiments/fixtures/sample-summary.txt&quot;')">Summarize Sample</button>
        <button class="qa-item" onclick="sendPromptFromPanel('find large files')">Large Files</button>
      </div>
    </div>
  </div>
  ...
  <script>
    ...
    async function handleSend() {
      ...
      document.getElementById('char-counter').style.color = 'var(--text-dim)';
    ...
    document.addEventListener('mousedown', closeQuickActionsOnOutsideClick);
    ...
    async function fetchStatus() {
      ...
      try {
        ...
        if (res.ok) {
          ...
          platformText.textContent = `${os} ${machine}`.trim();
        }
      } catch (e) {
        // Keep the default "Connected" label if status lookup fails.
      }
    }
    ...
    // Auto resize textarea and update the character counter.
